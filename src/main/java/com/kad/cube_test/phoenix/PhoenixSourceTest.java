...
public class PhoenixSourceTest {
...
//        initHiveCatalog(tableEnv);      // 初始化 Hive Catalog
        ...
        String sql = "select * from `phoenix`.`cube_phx`.ods_om_om_order";
        tableEnv.toAppendStream(tableEnv.sqlQuery(sql), Row.class).print("phoenix source");
        streamEnv.execute();
    }
