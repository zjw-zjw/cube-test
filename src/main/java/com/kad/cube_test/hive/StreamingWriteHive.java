...
public class StreamingWriteHive{
    public static void main(String[] args) throws Exception{
        ...
        tEnv.useDatabase("test_hive");

        tEnv.createTemporaryView("users", dataStream);

//      如果hive中已经存在了相应的表，则这段代码省略
        tEnv.getConfig().setSqlDialect(SqlDialect.HIVE);
        String dropTableSql = "DROP TABLE IF EXISTS `myhive`.`test_hive`.fs_table3";
		String hiveSql = "CREATE TABLE fs_table3 (\n" +
		                 "  user_id STRING,\n" +
		                 "  order_amount DOUBLE" +
        ...
        String insertSql = "insert into fs_table3 SELECT userId, amount, " +
